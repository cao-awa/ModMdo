package com.github.zhuaidadaya.modmdo.network.forwarder.connection;
...
public class ModMdoClientConnection {
    ...
    public ModMdoClientConnection(MinecraftServer server, InetSocketAddress address, JSONObject loginData) {
        ...
        send(new CustomPayloadC2SPacket(DATA, new PacketByteBuf(Unpooled.buffer()).writeIdentifier(LOGIN).writeString(loginData.toString())));
