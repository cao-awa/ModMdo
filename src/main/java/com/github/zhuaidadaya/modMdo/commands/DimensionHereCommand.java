...
public class DimensionHereCommand {
    public int dhere(CommandContext<ServerCommandSource> context) {
        try {
            ...
            for(String o : p.getPlayerNames()) {
                ...
                p.getPlayer(o).sendMessage(new LiteralText(dimensionTips.getDimensionColor(dimension) + whoUseHere + "在" +
                                                           dimensionTips.getDimensionName(dimension) + ": [" + "x:" + x + ", " + "y:" + y + ", " + "z:" + z + "]"), false);
            }
            return 1;
        } catch (Exception e) {
            return -1;
