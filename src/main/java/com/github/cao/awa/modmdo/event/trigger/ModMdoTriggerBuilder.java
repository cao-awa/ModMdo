...
public class ModMdoTriggerBuilder {
    public static final Object2ObjectOpenHashMap<String, String> classMap = EntrustEnvironment.operation(
            ...
            map -> {
                ...
                map.put(
                        ...
                        TntEntity.class.getName()
                );
                map.put(
                        "net.minecraft.entity.EyeOfEnderEntity",
                        EyeOfEnderEntity.class.getName()
                );
                map.put(
                        "net.minecraft.entity.ExperienceOrbEntity",
                        ExperienceOrbEntity.class.getName()
                );
                map.put(
                ...
                map.put(
                        ...
                        LightningEntity.class.getName()
                );
            }
    );
    private static final Logger LOGGER = LogManager.getLogger("ModMdoTriggerBuilder");
    public final ObjectArrayList<String> events = EntrustEnvironment.operation(
            new ObjectArrayList<>(),
            list -> {
    ...
    public final ObjectArrayList<String> targeted = EntrustEnvironment.operation(
            new ObjectArrayList<>(),
            list -> {
                list.add(DisconnectTrigger.class.getName());
                list.add(KillEntityTrigger.class.getName());
    ...
    public void register(JSONObject json, File trace) {
        ...
        if (e.containsKey("enable") && ! e.getBoolean("enable")) {
            return;
        }
        ...
        try {
            ...
            if (register == null || ! targeted.contains(name)) {
                ...
                register.register(
                        event -> prepareTargeted(
                                e,
                                (EntityTargetedEvent<?>) event,
                                trace
                        ),
                        SharedVariables.extras.getExtra(
                                ModMdo.class,
            ...
            throw new IllegalArgumentException(
                    "Event \"" + name + "\" not found, may you got key it wrong? will be not register this event",
                    ex
            );
        }
    }
    ...
    public void prepareTargeted(JSONObject event, EntityTargetedEvent<?> targeted, File trace) {
        ...
        if (targeted.getTargeted()
                    .size() > 1 || instance == null || instance.equals("") || EntrustEnvironment.trys(
                () -> classMap.getOrDefault(
                                      instance,
                                      instance
                              )
                              .equals(targeted.getTargeted()
                                              .get(0)
                                              .getClass()
                                              .getName()),
                () -> true
        )) {
            TriggerSelector selector = event.containsKey("controller") ?
                                       controller(event.getJSONObject("controller")) :
                                       new AllSelector();
            OperationalInteger i = new OperationalInteger();
            selector.select(
                    event.getJSONObject("triggers"),
                    (name, json) -> {
                        Temporary action = () -> {
                            EntrustEnvironment.notNull(
                                    EntrustEnvironment.trys(
                                            () -> {
                                                TargetedTrigger<EntityTargetedEvent<?>> trigger = EntrustEnvironment.cast(Class.forName(json.getString("instanceof"))
                                    ...
                                    ModMdoEventTrigger::ensureAction
                            );
                            i.add();
                        };
                        ...
                        EntrustEnvironment.trys(
                                () -> {
                                    ...
                                    int wait = awaiting.getInteger("or-wait");
                                    SharedVariables.event.events.get(awaiting.getString("instanceof"))
                                                                .await(
                                                                        action,
                                                                        wait,
                                                                        trace
                ...
                (name, json) -> {
                    Temporary action = () -> {
                        EntrustEnvironment.notNull(
                                EntrustEnvironment.trys(
                                        () -> {
                                            ModMdoEventTrigger<ModMdoEvent<?>> trigger = EntrustEnvironment.cast(Class.forName(json.getString("instanceof"))
                                                                                                                      .getDeclaredConstructor()
                                                                                                                      .newInstance());
                                            return trigger.build(
                                                    targeted,
    ...
    public TriggerSelector controller(JSONObject json) {
        return EntrustEnvironment.trys(
                () -> {
                    TriggerSelector selector = (TriggerSelector) Class.forName(json.getString("instanceof"))
                                                                      .getDeclaredConstructor()
                                                                      .newInstance();
                    selector.build(json);
                    return selector;
