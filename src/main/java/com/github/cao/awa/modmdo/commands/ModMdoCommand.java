...
public class ModMdoCommand extends SimpleCommand {
    public ModMdoCommand register() {
        SharedVariables.commandRegister.register(literal("modmdo").executes(modmdo -> {
            ...
            if (SharedVariables.commandApplyToPlayer(1, SimpleCommandOperation.getPlayer(reject), reject)) {
                ...
                SimpleCommandOperation.sendFeedback(reject, new TranslatableTextContent("reject_reconnect.true.rule.format"));
            }
            return 1;
            ...
            SimpleCommandOperation.sendFeedback(getLanguage, new TranslatableTextContent("language.default", SharedVariables.getLanguage()));
            return 0;
        }).then(literal("zh_cn").executes(chinese -> {
            config.set("default_language", com.github.cao.awa.modmdo.lang.Language.ZH_CN);
            SharedVariables.saveVariables();
            SimpleCommandOperation.sendFeedback(chinese, new TranslatableTextContent("language.default", SharedVariables.getLanguage()));
            return 0;
            ...
            SimpleCommandOperation.sendFeedback(setDef, new TranslatableTextContent("enchantment.max.level.limit.all", level));
            return 0;
        })).then(literal("default").executes(recoveryAll -> {
            SharedVariables.enchantLevelController.allDefault();
            SimpleCommandOperation.sendFeedback(recoveryAll, new TranslatableTextContent("enchantment.max.level.limit.all.default"));
            return 0;
            ...
            SimpleCommandOperation.sendFeedback(getLimit, new TranslatableTextContent("enchantment.max.level.limit", name, level));
            SharedVariables.saveEnchantmentMaxLevel();
            return 0;
            ...
            SharedVariables.saveEnchantmentMaxLevel();
            SimpleCommandOperation.sendFeedback(setLimit, new TranslatableTextContent("enchantment.max.level.limit", name, level));
            return 0;
            ...
            SharedVariables.enchantLevelController.set(name, level);
            SharedVariables.saveEnchantmentMaxLevel();
            SimpleCommandOperation.sendFeedback(recoveryLevel, new TranslatableTextContent("enchantment.max.level.limit", name, level));
            return 0;
            ...
            SimpleCommandOperation.sendFeedback(getRejectNoFall, new TranslatableTextContent(SharedVariables.rejectNoFallCheat ? "player.no.fall.cheat.reject" : "player.no.fall.cheat.receive"));
            return 0;
        }).then(literal("enable").executes(reject -> {
            SharedVariables.rejectNoFallCheat = true;
            SharedVariables.saveVariables();
            SimpleCommandOperation.sendFeedback(reject, new TranslatableTextContent(SharedVariables.rejectNoFallCheat ? "player.no.fall.cheat.reject" : "player.no.fall.cheat.receive"));
            return 0;
        })).then(literal("disable").executes(receive -> {
            SharedVariables.rejectNoFallCheat = false;
            SharedVariables.saveVariables();
            SimpleCommandOperation.sendFeedback(receive, new TranslatableTextContent(SharedVariables.rejectNoFallCheat ? "player.no.fall.cheat.reject" : "player.no.fall.cheat.receive"));
            return 0;
            ...
            if (SharedVariables.whitelist.containsName(wl.getName())) {
                ...
                SimpleCommandOperation.sendFeedback(remove, new TranslatableTextContent("modmdo.whitelist.removed", wl.getName()));
                SharedVariables.updateWhitelistNames(SimpleCommandOperation.getServer(remove), true);
                SharedVariables.saveVariables();
                return 0;
            }
            SimpleCommandOperation.sendError(remove, new TranslatableTextContent("arguments.permanent.whitelist.not.registered"));
            return - 1;
            ...
            EntrustExecution.tryTemporary(pair.getRight()::disconnect, nullProcessor -> SimpleCommandOperation.sendError(disconnect, new TranslatableTextContent("modmdo.connection.not.found", pair.getLeft())));
            return 0;
            ...
            EntrustExecution.tryTemporary(pair.getRight()::sendTraffic, nullProcessor -> SimpleCommandOperation.sendError(test, new TranslatableTextContent("modmdo.connection.not.found", pair.getLeft())));
            return 0;
            ...
            SimpleCommandOperation.sendFeedback(format, new TranslatableTextContent("modmdo.connection.chatting.format", formatting.replace("%server", "TestServer").replace("%name", "PlayerName233").replace("%msg", "Hi!")));
            return 0;
            ...
            sendFeedback(e, new TranslatableTextContent("modmdo.event.reload.success", pair.getLeft(), pair.getRight()));
            return 0;
        }))));
        return this;
    }
    ...
    public void showWhitelist(CommandContext<ServerCommandSource> source) throws CommandSyntaxException {
        ...
        if (SharedVariables.whitelist.size() > 0) {
            ...
            SimpleCommandOperation.sendMessage(player, new TranslatableTextContent("commands.modmdo.whitelist.list", SharedVariables.whitelist.size(), builder.toString()), false);
        } else {
            SimpleCommandOperation.sendMessage(player, new TranslatableTextContent("commands.modmdo.whitelist.none"), false);

        }
    }

    public TranslatableTextContent formatConfigCachedReturnMessage(String config) {
        return new TranslatableTextContent(config + "." + SharedVariables.config.getConfigString(config) + ".rule.format");
    }

    public TranslatableTextContent formatConfigReturnMessage(String config) {
        return new TranslatableTextContent(config + "." + SharedVariables.config.getConfigString(config) + ".rule.format");
    }

    public TranslatableTextContent formatCheckerTimeLimit() {
        return new TranslatableTextContent("checker_time_limit.rule.format", config.getConfigInt("checker_time_limit"));
    }

    public TranslatableTextContent formatTickingEntitiesTick() {
        return new TranslatableTextContent(SharedVariables.cancelEntitiesTick ? "ticking.entities.false.rule.format" : "ticking.entities.true.rule.format");
    }

    public TranslatableTextContent formatItemDespawnTicks() {
        if (SharedVariables.itemDespawnAge > - 1) {
            return new TranslatableTextContent("item.despawn.ticks.rule.format", SharedVariables.itemDespawnAge);
        } else {
            return new TranslatableTextContent("item.despawn.ticks.false.rule.format", SharedVariables.itemDespawnAge);
        }
    }

    public TranslatableTextContent formatEnableHere() {
        return new TranslatableTextContent("here_command.true.rule.format");
    }

    public TranslatableTextContent formatDisableHere() {
        return new TranslatableTextContent("here_command.false.rule.format");
    }

    public TranslatableTextContent formatEnableSecureEnchant() {
        return new TranslatableTextContent("secure_enchant.true.rule.format");
    }

    public TranslatableTextContent formatDisableSecureEnchant() {
        return new TranslatableTextContent("secure_enchant.false.rule.format");
    }

    public TranslatableTextContent formatUseModMdoWhitelist() {
        return new TranslatableTextContent("modmdo_whitelist.true.rule.format");
    }

    public TranslatableTextContent formatDisableModMdoWhitelist() {
        return new TranslatableTextContent("modmdo_whitelist.false.rule.format");
    }

    public TranslatableTextContent formatDisableRejectReconnect() {
        return new TranslatableTextContent("reject_reconnect.reject.false.rule.format");
    }
    ...
    public TranslatableTextContent formatModMdoDescription(ServerPlayerEntity player) {
        ...
        return new TranslatableTextContent("modmdo.description", SharedVariables.MODMDO_VERSION_NAME, SharedVariables.RELEASE_TIME, modmdoVersion);
    }
}
